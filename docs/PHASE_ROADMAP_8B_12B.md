...
**Split (2026-08-27):** the phase above conflated two genuinely different problems. Split into **12b-client** (rebase correctness + the transport contract - fully synthetic-testable, fits this project's agent-driven loop) and **12b-server/transport** (the actual server/socket, presence rendering, reconnect - explicitly *not* attempted the same way, per the note above). The OT-vs-CRDT line item was resolved by recognizing it wasn't actually open: `mapOperation`/`baseRevision` (Phase 8c) already committed the architecture to OT; a CRDT would have discarded that work rather than built on it.

**Status (2026-08-27): 12b-client complete.** Pre-work (§1.1) found `mapOperation` - built in Phase 8c to satisfy the collab-readiness gate's assertion 5 - had **zero production callers** anywhere in the codebase before this phase, and its own test coverage asserted "doesn't throw," never actual transform correctness. TP1 (TransformationProperty-1) seeded-PRNG convergence testing found four real position-arithmetic bugs (two concurrent inserts/removes at coincidentally-equal offsets computed wrong results or corrupted the document outright) and fixed all four (`docs/bugs/mapoperation-position-arithmetic-gaps.md`). The same testing found four concurrent-edit shapes with no safe automatic resolution - `moveNode` vs. any structural change sharing its array, edits to content a merge (`mergedInto`/`retiredInto`) absorbs, and overlapping/interior deleteText-vs-insertText - all now explicit `RebaseConflictError`s rather than silent data loss or corruption (`docs/bugs/concurrent-rebase-non-transformable-conflicts.md`), a scope decision made explicitly with the product owner rather than picked silently. One narrower gap, initially disclosed rather than fixed, was escalated the next day (2026-08-28) once formally checked against TP1 and confirmed to be a genuine convergence violation, not merely cosmetic: a concurrent insertion exactly at a suggestion/comment mark's end boundary was order-dependent, a pre-existing inconsistency (predating this phase, never exercised before since `mapOperation` had no production callers) between `mapOperation`'s hardcoded range bias and `insertText`'s own boundary convention. Fixed by aligning the bias to `insertText`'s existing resolution rather than inventing a new one; confirmed scoped to only that ambiguous case via the full pre-existing suite passing unchanged (`docs/bugs/addmark-boundary-bias-inconsistent-with-inserttext.md`). A second required follow-up (2026-08-28, same severity-first process) checked concurrent native `mergeNode`/`splitNode` transforms - exactly the operation pair Phase 12a's merge-orphan work was built around, and never exercised by the generic sweep at all (its generator never produced these two types regardless of case count). Found three distinct outcomes: two `mergeNode`s retiring adjacent siblings crashed one way and silently diverged the other (`mergeNode.splitOffset` is a child-count payload that goes stale exactly like `deleteText.text` does, now an explicit conflict); a `mergeNode` concurrent with a `splitNode` of the exact node being retired converges correctly via an existing, genuinely-recomputed redirect (confirmed safe, not just observed); two `splitNode`s at the identical boundary had no tie-break at all (fixed the same way `insertNode`/`insertText` ties already were). A quick moveNode re-check found no new gap. `FoundationEditor.dispatch` now rebases-and-reapplies a stale transaction through a bounded, configurable revision log instead of unconditionally throwing `StaleTransactionError`; a transaction older than the retained window gets `ResyncRequiredError` instead of a silent partial rebase. `CollabTransport`/`PresenceUpdate` (`packages/react`) define the transport contract mirroring `MediaProvider`/`VersionProvider` exactly - no default implementation ships, and (a deliberate scope decision, not an oversight) neither is wired into `CanonicalEditorRuntime` this phase, since the actual send/receive integration (echo prevention, batching, when to send) depends on transport-specific characteristics `12b-server/transport` will supply. Single-writer fast path confirmed provably unchanged: the entire pre-existing suite (704 core / 130 react tests, full 3-browser e2e) passes unmodified, since nothing before this phase ever exercised the stale-`baseRevision` path at all. Verification: core 720/720 (+16), react 132/132 (+2), lint clean, full e2e 468 passed / 7 skipped / 2 failed (both the pre-existing, unrelated Firefox clipboard-seeding flake).
