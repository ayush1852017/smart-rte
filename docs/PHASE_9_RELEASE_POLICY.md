...
6. **`mammoth`'s bundled transitive `@xmldom/xmldom@0.8.11` is a reachable DoS in the DOCX import path** — see `docs/bugs/mammoth-bundles-vulnerable-transitive-xmldom.md` and "Dependency vulnerability scan" below. The originally recommended fix (a pnpm override) was tried and reverted — it broke DOCX import entirely, a worse outcome than the vulnerability itself. A real fix requires a different approach and remains undecided. **Blocking, still open** — this is exactly the class of finding this criterion list exists to catch before a real publish, not after.
...
**The recommended fix (a pnpm override forcing `@xmldom/xmldom` to `^0.9.11` tree-wide) was tried and reverted.** It worked correctly at the dependency-resolution level — `pnpm -r why @xmldom/xmldom` confirmed a single `0.9.11` everywhere, `pnpm audit` confirmed all 5 advisories gone — but broke DOCX import completely: every test hit `TypeError: DOMParser.parseFromString: the provided mimeType "undefined" is not valid.`, because mammoth's own internal call site calls `parseFromString(string)` with no `mimeType` argument, and `@xmldom/xmldom`'s entire `0.9.x` line (confirmed from `0.9.0` onward, not just `0.9.11`) validates `mimeType` *before* applying its own documented default, throwing instead of defaulting. This is a genuine inconsistency in `@xmldom/xmldom` itself (its JSDoc says `mimeType` defaults to `'application/xml'`; its implementation doesn't honor that when the argument is omitted), not something specific to mammoth or to this repo. No version within `0.9.x` avoids it. Reverted `package.json`/`pnpm-lock.yaml` to their pre-override state; confirmed mammoth back on `0.8.11` and the full test suite passing again at prior counts.

**Still not fixed.** Real options (none applied, all need explicit confirmation): patch/vendor mammoth's own xmldom call site to pass an explicit mimeType; replace `mammoth` with a different DOCX-to-HTML library; accept the DoS as a documented residual risk; report the mimeType-default inconsistency upstream to `@xmldom/xmldom` (independently worth doing, doesn't fix this repo). Full detail and reasoning for each: `docs/bugs/mammoth-bundles-vulnerable-transitive-xmldom.md`.
