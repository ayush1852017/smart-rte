...
7. **NEW, found while verifying item 6's fix: mammoth's own XML-to-node-tree conversion (`lib/xml/reader.js`'s `convertNode`/`convertElement`) has unguarded recursion, independent of `@xmldom/xmldom` entirely.** A deeply-nested real `.docx` still crashes `importDocxDocumentWithMammoth` with the same `RangeError: Maximum call stack size exceeded` symptom, via completely different code, pre-existing and unrelated to any version of `@xmldom/xmldom`. See `docs/bugs/mammoth-own-reader-unguarded-recursion-dos.md`. **Blocking, open** — same reasoning as item 6 originally had: a DoS in a routine, expected-use feature, not an edge case.

None of the still-open items block `-beta.1` itself — a pre-release channel exists precisely so real integration testing can happen against something installable before the above is fully closed. They block calling this **stable** and moving the npm `latest` tag to it. Item 7 in particular should be resolved before *any* publish, including `beta` — it is the direct successor to item 6's original concern, just with a different, previously-undisclosed root cause.
...
**Verifying against the real end-to-end PoC (a crafted `.docx` through the actual `importDocxDocumentWithMammoth` entry point, not just the isolated library call) surfaced a second, previously-undisclosed, unrelated bug**: mammoth's own `lib/xml/reader.js` has its own unguarded recursive tree-walker (`convertNode`/`convertElement`), completely independent of `@xmldom/xmldom` and untouched by this patch. A deeply-nested real `.docx` still crashes the import — via different code, for a different reason, pre-existing before any of this session's changes. This is now item 7 above and `docs/bugs/mammoth-own-reader-unguarded-recursion-dos.md`, not a continuation of this finding.

Full detail (all three attempts, exact patch diffs, verification at each step, the drafted-but-unfiled upstream issue — no `gh` CLI available in this environment) in `docs/bugs/mammoth-bundles-vulnerable-transitive-xmldom.md`.
