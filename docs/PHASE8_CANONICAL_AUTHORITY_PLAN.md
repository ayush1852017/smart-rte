...
1. Retire `canonical-clipboard-dom-insert` when paste/cut/drop commit directly
   to the retained canonical editor instead of calling `execCommand`.
2. Retire `canonical-inline-dom-roundtrip` after product input, stored marks,
   links, and atom composition use persistent canonical state.
3. Retire `canonical-block-dom-roundtrip` after block/list/table toolbar and
   keyboard commands resolve scopes from the retained editor.
4. Retire `canonical-list-dom-roundtrip` after fragment insertion and clipboard
   normalization no longer parse the live list DOM.

The inventory must move `4 -> 3 -> 2 -> 1 -> 0` in reviewed commits. No replacement
