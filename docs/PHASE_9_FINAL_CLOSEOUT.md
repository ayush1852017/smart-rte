...
**Phase 9 is still not fully closed.** The third finding is open at the root, though its practical risk is closed:

- **`docs/bugs/mammoth-office-xml-reader-collapse-alternate-content-recursion.md` (open at the root; practically mitigated).** A third independent unguarded-recursion function in mammoth's own code, found only by fixing the second one and re-running the real attack. The new nesting-depth guard (`packages/core/src/foundation/formats/docx/nestingGuard.ts`) prevents any payload deep enough to reach it from ever reaching mammoth at all — so the *practical* DoS risk via `importDocxDocumentWithMammoth` is closed as of this round, even though the underlying recursive function itself is untouched. The gap: the guard only checks `word/document.xml`; other XML parts mammoth reads aren't covered, and the root cause is immediately live again if the guard is ever removed without someone realizing what it was covering for.
- **The bigger-picture question, raised explicitly in `docs/PHASE_9_RELEASE_POLICY.md`'s item 8:** three independent instances of the same bug class, found sequentially by fixing one and immediately hitting the next, is evidence of a pattern in mammoth's codebase rather than three unrelated coincidences. Whether that warrants a systematic audit of mammoth before trusting it for `latest` (rather than continuing to whack-a-mole individual functions as real-attack testing surfaces them) is an open decision, not evaluated in this pass.
