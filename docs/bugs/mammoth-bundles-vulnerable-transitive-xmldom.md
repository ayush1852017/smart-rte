...
`docs/bugs/mammoth-own-reader-unguarded-recursion-dos.md` — found while verifying this fix; a genuinely separate, previously-undisclosed DoS in mammoth's own code, same symptom, unrelated root cause. **Update 2026-08-16: now also fixed** (iterative rewrite of `reader.js`'s `convertNode`/`convertElement`, verified to 500,000 levels). That fix's own verification pass then surfaced a *third* instance, `docs/bugs/mammoth-office-xml-reader-collapse-alternate-content-recursion.md` — root cause still open there, but practically mitigated by the depth guard added in that same pass. This entry's own status stays **Fixed**: the specific 5 disclosed `@xmldom/xmldom@0.8.11` advisories it describes are fully resolved and unaffected by the later findings, which are independent bugs discovered incidentally while verifying this one. This is the first dependency-supply-chain finding in this project's history — distinct in kind from the application-logic bugs this ledger otherwise records.
