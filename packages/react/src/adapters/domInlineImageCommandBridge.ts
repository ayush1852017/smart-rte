...
export interface DomInlineImageInput { src: string; alt?: string; title?: string; width?: number; height?: number; align?: "center" | "left" | "right"; status?: "pending" | "ready" | "error"; uploadId?: string; error?: string }
...
export const executeDomInlineImageCommand = (
  ...
  type: "image",
  attrs: { src: input.src, alt: input.alt ?? "", status: input.status || "ready", ...(input.uploadId ? { uploadId: input.uploadId } : {}), ...(input.error ? { error: input.error } : {}), ...(input.title ? { title: input.title } : {}), ...(input.width ? { width: input.width } : {}), ...(input.height ? { height: input.height } : {}) },
