...
/**
 * Fidelity: `lossy` for everything - PDF has no structural document model to
 * recover, only positioned text runs. This is a best-effort layout
 * reconstruction (lines -> paragraphs/headings/lists/tables via position
 * heuristics), not a round-trip format. Matches the standing scope decision
 * since Phase 1.
 */
export const reconstructPdfPages = (pages: readonly PdfPageSnapshot[]): PdfImportResult => {
  let layoutHtml = "";
  pages.forEach((page) => {
  ...
  return { document: parseCanonicalListHtml(layoutHtml), layoutHtml, pages: pages.length };
};

export const importPdfDocument = async (arrayBuffer: ArrayBuffer): Promise<PdfImportResult> => {
  ...
  for (let index = 1; index <= pdf.numPages; index += 1) {
    ...
    pages.push({
      ...
      items: (content.items as Array<{ str?: string; transform: number[]; width?: number; fontName: string }>).map((item) => ({
        text: item.str || "",
        x: item.transform[4],
        y: item.transform[5],
        width: item.width || 0,
